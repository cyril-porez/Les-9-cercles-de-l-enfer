...
int LPTF_Socket::send(char* buffer, int bufferSize)
{
    if (::send(sockfd, buffer, bufferSize, 0) == SOCKET_ERROR)
    {
        return 1;
    }
...
int LPTF_Socket::send(SOCKET clientSock, char* buffer, int bufferSize)
{
    if (::send(clientSock, buffer, bufferSize, 0) == SOCKET_ERROR)
    {
        return 1;
    }
    // std::cout << "Message sent successfully: " << message << std::endl;
    ...
    for (size_t i = 0; i < clientSockets.size(); ++i)
    {
        if (FD_ISSET(clientSockets[i], &fdList))
        {
            ...
            else
            {
                ...
                send(clientSockets[i], buffer, sizeof(buffer));
            }
        }
    }
}

int LPTF_Socket::receiveMessage(Message &msg)
{
    char buffer[sizeof(Message)];
    int result = recv(buffer, sizeof(buffer));
    if (result != 0)
    {
    ...
    std::cout << "Message successfully received and deserialized" << std::endl;
    return 0;
}

int LPTF_Socket::sendMessage(Message &msg)
{
    char buffer[sizeof(Message)];
    int result = send(buffer, sizeof(buffer));
    if (result != 0)
    {
        std::cerr << "Error receiving data" << std::endl;
        return result;
